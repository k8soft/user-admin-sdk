...
dest=$ROOT/../projects/user-admin-sdk/src/lib/
...
#     -o $ROOT/../$dest \
#     --additional-properties=platform=browser,npmName=@k8soft/user-admin-sdk,npmVersion=$version,providedIn=platform
...
# perl -pi -e 'BEGIN{undef $/;} s/user-admin-sdk@(\d+)\.(\d+)\.(\d+)/user-admin-sdk/smg' $ROOT/../README.md
...
# # push to github
# $ROOT/gitpush.sh k8scommerce user-admin-sdk "update to version ${version}" "github.com"
